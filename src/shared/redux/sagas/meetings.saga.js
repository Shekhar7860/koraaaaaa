...
function* createTask(action) {
  try {
    const response = yield call(invokeAPICall, {
      url: '/api/1.1/ka/users/:userId/tasks',
      method: API_CONST.POST,
      data: action.payload,
    });
    if (response.data) {
    ...
    yield put({
      type: Type.CREATE_TASK_SUCCESSFUL,
      payload: response.data,
    });
  } catch (e) {
    console.log('Task cannot be created', e.message);
    yield put({
...
export function* meetingSaga() {
  ...
  yield all([takeLatest(Type.CREATE_TASK, createTask)]);
