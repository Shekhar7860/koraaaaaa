...
export function getBoardTables(workspaceId) {
  return new Promise(async (resolve, reject) => {
    try {
      const db = database.active;
      const whereClause = [
        Q.where('wsId', workspaceId),
        Q.where('type', 'table'),
      ];
      const boardsCollection = db.collections.get(Entity.Boards);
      const results = await boardsCollection.query(...whereClause).fetch();
      if (results?.length > 0) {
        let board = results;
        resolve(board);
      } else {
      ...
      console.log('error in getBoardTables :', e.message);
      resolve(null);
    }
  });
}
