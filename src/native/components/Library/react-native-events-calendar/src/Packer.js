...
const CALENDER_HEIGHT = 1300;
const offset = CALENDER_HEIGHT / 24;

function buildEvent(column, left, width) {
  const startTime = moment(column.duration.start);
  const endTime = column.duration.end
    ? moment(column.duration.end)
...
function collision(a, b) {
  return a.duration.end > b.duration.start && a.duration.start < b.duration.end;
}
...
function populateEvents(events, screenWidth) {
  ...
  events = events
    ...
    .sort(function (a, b) {
      ...
      if (a.duration.start > b.duration.start) return 1;
      if (a.duration.end < b.duration.end) return -1;
      if (a.duration.end > b.duration.end) return 1;
      return 0;
    });
  ...
  events.forEach(function (ev, index) {
    if (lastEnd !== null && ev.duration.start >= lastEnd) {
